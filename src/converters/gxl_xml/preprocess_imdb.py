...
def main():
    ...
    for idx, data in enumerate(dataset, start=1):
        ...
        gxl_tree = create_gxl_for_graph_imdb(
            g_id, node_ids, local_ids, edges, graph_label
        )
    ...
    print(
        f"Conversion complete. {len(collection_entries)} graphs written to '{args.output_dir}'."
    )
