        ...
        while i < len(options):
            if options[i] == "-log2c":
                ...
                if options[i] == "null":
                    ...
                    self.c_begin, self.c_end, self.c_step = map(
                        float, options[i].split(",")
                    )
                ...
                if options[i] == "null":
                    ...
                    self.g_begin, self.g_end, self.g_step = map(
                        float, options[i].split(",")
                    )
            elif options[i] == "-v":
                i = i + 1
...
def redraw(db, best_param, gnuplot, options, tofile=False):
    ...
    gnuplot.write(
        'set label "Best log2(C) = {0}  log2(gamma) = {1}  accuracy = {2}%" \
				  at screen 0.5,0.85 center\n'.format(
            best_log2c, best_log2g, best_rate
        ).encode()
    )
    gnuplot.write(
        'set label "C = {0}  gamma = {1}" at screen 0.5,0.8 center\n'.format(
...
class TelnetWorker(Worker):
    def __init__(
        self, name, job_queue, result_queue, host, username, password, options
    ):
        Worker.__init__(self, name, job_queue, result_queue, options)
        self.host = host
...
def find_parameters(dataset_pathname, options=""):
    ...
    if options.gnuplot_pathname:
        gnuplot = Popen(
            options.gnuplot_pathname, stdin=PIPE, stdout=PIPE, stderr=PIPE
        ).stdin
    else:
        gnuplot = None
    ...
    if telnet_workers:
        ...
        for host in telnet_workers:
            worker = TelnetWorker(
                host, job_queue, result_queue, host, username, password, options
            )
            worker.start()
    ...
    for c, g in resumed_jobs:
        ...
        best_c, best_g, best_rate = update_param(
            c, g, rate, best_c, best_g, best_rate, "resumed", True
        )
        ...
        for c, g in line:
            while (c, g) not in done_jobs:
                ...
                if (c1, g1) not in resumed_jobs:
                    best_c, best_g, best_rate = update_param(
                        c1, g1, rate1, best_c, best_g, best_rate, worker, False
                    )
