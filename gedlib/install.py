...
def append_ged_env_hpp(identifier, node_id_type, node_label_type, edge_label_type):
    ...
    with open("src/env/ged_env.hpp", "r") as f:
        for line in f:
            if line.startswith("#ifdef ") and not line.startswith(
...
def append_cmake_lists(identifier):
    ...
    append = (
        ...
        + identifier.lower()
        + 'gedlib PROPERTIES SUFFIX ".so")\n'
    )
    append = (
        append
        + "target_link_libraries("
        + identifier.lower()
        + "gedlib nomad doublefann svm)\n"
    )
    ...
    append = (
        ...
        + identifier.lower()
        + "gedlib POST_BUILD COMMAND install_name_tool -change libnomad.so ${NOMAD_HOME}/lib/libnomad.so ${LIBRARY_OUTPUT_PATH}/lib"
        + identifier.lower()
        + "gedlib.so)\n"
    )
    append = (
        ...
        + "  add_custom_command(TARGET "
        + identifier.lower()
        + "gedlib POST_BUILD COMMAND install_name_tool -change libdoublefann.2.dylib ${FANN_HOME}/lib/libdoublefann.2.dylib ${LIBRARY_OUTPUT_PATH}/lib"
        + identifier.lower()
        + "gedlib.so)\n"
    )
    append = (
        ...
        + "  add_custom_command(TARGET "
        + identifier.lower()
        + "gedlib POST_BUILD COMMAND install_name_tool -change libsvm.so ${LIBSVM_HOME}/libsvm.so ${LIBRARY_OUTPUT_PATH}/lib"
        + identifier.lower()
        + "gedlib.so)\n"
    )
    ...
    append = (
        ...
        + "  add_custom_command(TARGET "
        + identifier.lower()
        + "gedlib POST_BUILD COMMAND install_name_tool -change libomp.dylib ${OMP_HOME}/lib/libomp.dylib ${LIBRARY_OUTPUT_PATH}/lib"
        + identifier.lower()
        + "gedlib.so)\n"
    )
    append = append + "endif()\n"
    ...
    with open("src/CMakeLists.txt", "r") as f:
        for line in f:
            ...
            if line.startswith("add_library(") and not line.startswith(
                "add_library(gxlgedlib"
            ):
                delete_line = 9
...
def create_template_instantiation(
    identifier, node_id_type, node_label_type, edge_label_type
):
...
def parse_custom_types(custom_types):
    if len(custom_types.split(",")) != 4:
        raise Exception(
            'Invalid argument "'
            + args.lib
            + "\" for option lib. Usage: python install.py [--lib gxl|<indentifier-different-from-'gxl'>,<node-ID-type>,<node-label-type>,<edge-label-type>] [...]"
        )
...
def create_directories():
    ...
    commands = (
        commands + "mkdir -p tests/ijprai2020/bin; mkdir -p tests/ijprai2020/output; "
    )
    commands = (
        commands
        + "mkdir -p tests/vldbj2020/bin; mkdir -p tests/vldbj2020/ini; mkdir -p tests/vldbj2020/results; "
    )
    commands = (
        commands
        + "mkdir -p median/bin; mkdir -p median/output; mkdir -p median/data; mkdir -p median/data/Letter; mkdir -p median/data/Mutagenicity; "
    )
    commands = (
        commands + "mkdir -p tests/sspr2018/bin; mkdir -p tests/sspr2018/output; "
    )
    commands = (
        commands + "mkdir -p tests/unit_tests/bin; mkdir -p tests/unit_tests/output"
    )
...
def determine_gurobi_dylib(gurobi_root):
    if not os.path.isdir(gurobi_root):
        raise Exception(
            'Invalid argument "'
            + gurobi_root
            + '" for option gurobi: not a directory. Usage: python install.py [--gurobi <path-to-root-directory-of-Gurobi>] [...]'
        )
    return (
        "gurobi"
        + gurobi_root[gurobi_root.index("gurobi") + 6 : gurobi_root.index("gurobi") + 8]
    )
...
def determine_gurobi_statlib(gurobi_root):
    ...
    if platform.system() == "Linux":
        info = {
            ...
            for item in subprocess.check_output("lsb_release -a", shell=True)
            .decode("utf-8")
            .split("\n")[:-1]
        }
        if info["Distributor ID"] == "Ubuntu" and (
            info["Release"] == "16.04" or info["Release"] == "18.04"
        ):
...
def build_gedlib(args):
    ...
    if args.lib and args.lib != "gxl":
        identifier, node_id_type, node_label_type, edge_label_type = parse_custom_types(
            args.lib
        )
    ...
    if not os.path.isfile("build/Makefile"):
        ...
        if args.gurobi:
            commands = (
                ...
                + args.gurobi
                + " -DGUROBI_DYLIB="
                + determine_gurobi_dylib(args.gurobi)
                + " -DGUROBI_STATLIB="
                + determine_gurobi_statlib(args.gurobi)
            )
        if platform.system() == "Darwin":
            commands = (
                commands
                + " -DOMP_HOME="
                + check_output("brew --prefix libomp", shell=True).decode("utf-8")
            )
        call(commands, shell=True)
...
parser = argparse.ArgumentParser(
    description="Installs GEDLIB and its dependencies unless they have already been installed.",
    epilog="If called without arguments, only the dependencies are installed.",
)
parser.add_argument("--doc", help="build documentation", action="store_true")
parser.add_argument(
    "--lib",
    help="build shared library",
    metavar="gxl|<indentifier>,<UserNodeID>,<UserNodeLabel>,<UserEdgeLabel>",
)
parser.add_argument(
    "--tests",
    help="build test executables",
    metavar="all|unit_tests|ged_env_tests|lsap_solver_tests|ijprai2020|pvldb2020|sspr2018|vldbj2020|vldbj_train_ml|vldbj_test_lsape_based_methods|vldbj_test_lp_based_methods|vldbj_test_ls_based_methods|vldbj_test_misc_methods|median|cluster|bst|ibd",
    choices=[
        "all",
        "unit_tests",
        "ged_env_tests",
...
parser.add_argument(
    "--gurobi",
    metavar="<GUROBI_ROOT>",
    help="specify path to directory containing Gurobi",
)
parser.add_argument("--debug", help="build in debug mode", action="store_true")
parser.add_argument(
    "--clean",
    help="clean build directory and update makefile before build",
    action="store_true",
)
