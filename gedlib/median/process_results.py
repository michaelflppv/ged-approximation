...
def aggregate_results(csv_file):
    ...
    agg_rs = grouped.agg(
        {
            "time": np.mean,
            "time_init": np.mean,
            "time_converged": np.mean,
            "sod": np.mean,
            "sod_init": np.mean,
            "sod_converged": np.mean,
            "itrs": concat_lists,
            "state": as_list,
        }
    )
    ...
    agg_rs["itrs_mean"] = agg_rs.itrs.apply(
        lambda l: np.mean(l) if len(l) > 0 else None
    )
    agg_rs["itrs_lower_quartile"] = agg_rs.itrs.apply(
        lambda l: np.quantile(l, 0.25) if len(l) > 0 else None
    )
    agg_rs["itrs_median"] = agg_rs.itrs.apply(
        lambda l: np.quantile(l, 0.5) if len(l) > 0 else None
    )
    agg_rs["itrs_upper_quartile"] = agg_rs.itrs.apply(
        lambda l: np.quantile(l, 0.75) if len(l) > 0 else None
    )
    agg_rs["ratio_initialized"] = agg_rs.state.apply(
        lambda l: ratio_greater(l, 0) if len(l) > 0 else None
    )
    agg_rs["ratio_converged"] = agg_rs.state.apply(
        lambda l: ratio_greater(l, 1) if len(l) > 0 else None
    )
    agg_rs["ratio_refined"] = agg_rs.state.apply(
        lambda l: ratio_greater(l, 2) if len(l) > 0 else None
    )
    return agg_rs
    ...
    for algo in algos:
        for init_type in init_types:
            for num_inits in nums_inits:
                ...
                for col in cols:
                    row_rs[prefix + col] = agg_rs[
                        (agg_rs.algo == algo)
                        & (agg_rs.init_type == init_type)
                        & (agg_rs.num_inits == num_inits)
                    ][col].reset_index()[col]
    return row_rs
    ...
    for algo in algos:
        for percent in percents:
            ...
            for col in cols:
                row_rs[prefix + col] = agg_rs[
                    (agg_rs.algo == algo)
                    & (agg_rs.init_type == "RANDOM")
                    & (agg_rs.percent == percent)
                ][col].reset_index()[col]
    return row_rs


def create_init_plots(
    rs_vs_num_inits_letter, rs_vs_num_inits_aids, rs_vs_num_inits_muta
):
    tikz = open("../img/effect_num_inits.tex", "w")
    tikz.write("%!TEX root = ../main.tex\n")
    tikz.write("\\begin{tikzpicture}\n")
    tikz.write(
        "\\begin{groupplot}[group style={group name=groupplot, group size=3 by 2, horizontal sep=1.1cm, vertical sep=1.5cm},\n"
    )
    ...
    create_lineplots_inits_vs_sod(
        rs_vs_num_inits_letter, tikz, "\\letter (\\SI{90}{\\percent})", True
    )
    create_lineplots_inits_vs_sod(
        rs_vs_num_inits_aids, tikz, "\\aids (\\SI{90}{\\percent})"
    )
    create_lineplots_inits_vs_sod(
        rs_vs_num_inits_muta, tikz, "\\muta (\\SI{90}{\\percent})"
    )
    create_barplots_inits_vs_state(
        rs_vs_num_inits_letter, tikz, "\\letter (\\SI{90}{\\percent})"
    )
    create_barplots_inits_vs_state(
        rs_vs_num_inits_aids, tikz, "\\aids (\\SI{90}{\\percent})"
    )
...
def create_config_plots(rs_vs_percent_letter, rs_vs_percent_aids, rs_vs_percent_muta):
    ...
    tikz.write("legend style={align=left, draw=none, column sep=.5ex},\n")
    tikz.write("/tikz/font=\small]\n")
    create_paretoplots_sod_vs_time(
        rs_vs_percent_letter, tikz, "\\letter (\\SI{90}{\\percent})", True
    )
    create_paretoplots_sod_vs_time(
        rs_vs_percent_aids, tikz, "\\aids (\\SI{90}{\\percent})"
    )
    create_paretoplots_sod_vs_time(
        rs_vs_percent_muta, tikz, "\\muta (\\SI{90}{\\percent})"
    )
...
def create_boxplots_inits_vs_itrs(rs_vs_num_inits, tikz, title):
    ...
    for algo in ["BRANCH_FAST", "REFINE", "IPFP"]:
        ...
        for num_inits in [2**i for i in range(6)]:
            ...
            tikz.write(
                "lower whisker="
                + str(rs_vs_num_inits.at[index[num_inits], col_prefix + "min"])
                + ",\n"
            )
            tikz.write(
                "lower quartile="
                + str(
                    rs_vs_num_inits.at[index[num_inits], col_prefix + "lower_quartile"]
                )
                + ",\n"
            )
            tikz.write(
                "median="
                + str(rs_vs_num_inits.at[index[num_inits], col_prefix + "median"])
                + ",\n"
            )
            tikz.write(
                "upper quartile="
                + str(
                    rs_vs_num_inits.at[index[num_inits], col_prefix + "upper_quartile"]
                )
                + ",\n"
            )
            tikz.write(
                "upper whisker="
                + str(rs_vs_num_inits.at[index[num_inits], col_prefix + "max"])
                + "\n"
            )
            tikz.write("},\n")
...
def create_paretoplots_sod_vs_time(rs_vs_percent, tikz, title, legend=False):
    ...
    marks = {
        ...
        "MEAN": "mark=triangle",
        "MEDOID": "mark=diamond",
        "RANDOM": "mark=pentagon",
        "MAXconv": "mark=+",
        "MINconv": "mark=x",
        "MEANconv": "mark=-",
        "MEDOIDconv": "mark=|",
        "RANDOMconv": "mark=star",
    }
    colors = {
        "BRANCH_FAST": "draw=BRANCHFAST",
        "REFINE": "draw=REFINE",
        "IPFP": "draw=IPFP",
    }
    num_inits = {
    ...
    if legend:
        for algo in ["BRANCH_FAST", "REFINE", "IPFP"]:
            for init_type in ["MAX", "MIN", "MEAN", "MEDOID", "RANDOM"]:
                style = (
                    "only marks, mark size=3pt, thick, "
                    + marks[init_type]
                    + ", "
                    + colors[algo]
                )
                ...
                style = (
                    "only marks, mark size=3pt, "
                    + marks[init_type + "conv"]
                    + ", "
                    + colors[algo]
                )
                tikz.write("\\addlegendimage{" + style + "};\n")
...
def create_lineplots_max_itrs_vs_percent(rs_vs_percent, tikz, title, legend=False):
    ...
    for algo in ["BRANCH_FAST", "REFINE", "IPFP"]:
        for init_type in ["MAX", "MIN", "MEAN", "MEDOID", "RANDOM"]:
            ...
            for pos in range(9):
                tikz.write(
                    str(10 * (pos + 1)) + "," + str(rs_vs_percent.at[pos, col]) + "\n"
                )
            tikz.write("};\n")
...
def create_lineplots_inits_vs_sod(rs_vs_num_inits, tikz, title, legend=False):
    ...
    if legend:
        ...
        for algo in ["BRANCH_FAST", "REFINE", "IPFP"]:
            tikz.write(
                "\\addlegendimage{fill="
                + color[algo]
                + ", postaction={pattern=north east lines}, area legend};\n"
            )
    ...
    for algo in ["BRANCH_FAST", "REFINE", "IPFP"]:
        ...
        tikz.write(
            "\\addplot[color="
            + color[algo]
            + ", thick] table [x=x, y=y, col sep=comma] {\n"
        )
        tikz.write("x,y\n")
        for num_inits in [2**i for i in range(6)]:
            tikz.write(
                str(num_inits)
                + ","
                + str(rs_vs_num_inits.at[index[num_inits], col])
                + "\n"
            )
        tikz.write("};\n")
...
def create_barplots_inits_vs_state(rs_vs_num_inits, tikz, title):
    ...
    for algo in ["BRANCH_FAST", "REFINE", "IPFP"]:
        ...
        for num_inits in [2**i for i in range(6)]:
            ratio_refined = (
                rs_vs_num_inits.at[index[num_inits], col_prefix + "_refined"] * 100
            )
            ratio_converged = (
                rs_vs_num_inits.at[index[num_inits], col_prefix + "_converged"] * 100
            )
            ratio_init = (
                rs_vs_num_inits.at[index[num_inits], col_prefix + "_initialized"] * 100
            )
            ...
            if ratio_refined > 0:
                tikz.write("\\addplot+[ybar,\n")
                tikz.write(
                    "fill=" + color[algo] + ", postaction={pattern=north east lines},\n"
                )
                tikz.write("draw=none,\n")
                tikz.write(
                    "] coordinates {("
