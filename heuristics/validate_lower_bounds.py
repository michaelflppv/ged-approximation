...
def run_ged_executable(graph_file1, graph_file2, ged_executable):
    ...
    cmd = [
        ...
        "astar",
        "-l",
        "BMao",  # Fixed heuristic name in command.
        "-t",
        "-1",
        "-g",
    ]
...
def process_pair(
    pair, apply_heuristic, lb_df_filter, dataset, threshold, ged_executable
):
    """
...
def save_results_excel(output_file, baseline_results, heuristic_results_dict, summary):
    ...
    def sanitize_sheet_name(sheet_name):
        ...
        return re.sub(invalid_chars, "_", sheet_name)[
            :31
    ...
    with ExcelWriter(output_file, engine="openpyxl") as writer:
        DataFrame(baseline_results).to_excel(
            writer, sheet_name="Without_Heuristics", index=False
        )
...
def main():
    ...
    heuristic_files = [
        f
        for f in os.listdir(lb_folder)
        if f.endswith(".xlsx") and f.startswith(f"{dataset}_")
    ]
    if not heuristic_files:
        print(
            f"No heuristic Excel files found in {lb_folder} for dataset {dataset}. Exiting."
        )
        sys.exit(1)
    ...
    for file in heuristic_files:
        ...
        print(
            f"Experiment with heuristic '{heuristic_name}' completed in {time_withheur:.2f} seconds. "
    ...
    # -------------------------------
    save_results_excel(
        output_excel, baseline_results, heuristic_results_dict, summary_list
    )
